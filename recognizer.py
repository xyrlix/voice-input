...
class VoiceRecognizer:
    ...
    def load_model(self):
        ...
        try:
            ...
            print("   或: pip install git+https://github.com/openai/whisper.git")
            return False

        try:
            # 加载模型
            self.model = whisper.load_model(self.model_file, download_root=str(MODEL_DIR))
    ...
    def recognize_from_file(self, audio_path: str) -> str:
        ...
        try:
            ...
            print(f"   ⏱️ 耗时: {duration:.1f}秒，置信度: {result.get('confidence', 0):.2%}")
    ...
    def recognize_from_bytes(self, audio_data: bytes, sample_rate: int = 16000) -> str:
        """从音频字节数据直接识别（无需保存文件）"""
        if not self.load_model():
            return "❌ Whisper 模型未加载"
