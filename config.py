...
def config_to_str(config: dict = None) -> str:
    ...
    for key, value in sorted(config.items()):
        if key == "hotkey":
            ...
            lang_name = {"zh": "中文", "en": "英文", "auto": "自动识别"}.get(value, value)
            lines.append(f"🌐 识别语言: {lang_name}")
        elif key == "model_name":
            size_mb = MODEL_SIZES.get(value, "?")
            lines.append(f"🧠 识别模型: {value} (~{size_mb}MB)")
        elif key == "input_mode":
            mode_name = {"push_to_talk": "按住说话", "toggle": "开关模式"}.get(value, value)
            lines.append(f"🎤 录音模式: {mode_name}")
        elif key == "output_method":
            method_name = {"typing": "模拟打字", "clipboard": "复制粘贴"}.get(value, value)
